...
class BillDetailTableViewController: UITableViewController, UITextFieldDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let nav = segue.destination as? UINavigationController, let destination = nav.viewControllers.first as? BillListTableViewController {
            destination.updateSnapshot()
        }
    }
    ...
    func presentNeedAuthorizationAlert() {
        let alert = UIAlertController(
            ...
            message: "Please enable notifications in iOS Settings to use reminders.",
